...
// Se muestra la fecha en curso.
  echo ("Reserva para el día: ".$diaActual." del ".$mesActual." de ".$annioActual.salto);
// Se leen los datos de la cita, en la tabla "citas".
  $consulta="SELECT hora_reserva, asunto_reserva FROM tbl_reserva WHERE id_reserva='".$_POST["reservaSeleccionada"]."';";
...
clave primaria del campo empleado en la selección. */
  $datosDeLaCita=mysqli_fetch_array($hacerConsulta, MYSQL_ASSOC);
/* Se asignan los datos actuales de la cita a variables de memoria, se libera el cursor y se cierrqa la BBDD. */
  $horaActual=substr($datosDeLaReserva["hora_reserva"],0,2);
  $minutoActual=substr($datosDeLaReserva["hora_reserva"],3,2);
  $asuntoActual=$datosDeLaReserva["asunto_reserva"];
  @mysql_free_result ($hacerConsulta);
  mysql_close($conexion);
?>
<html>
  <head>
    <title>Cambiar una reserva existente</title>
...
/* La siguiente función envía el formulario a donde proceda.*/
      function mandar (resultado){
        if (resultado){
          document.formularioNuevaReserva.action="grabarCambios.php";
        } else {
          document.formularioNuevaReserva.action="index.php";
        }
        document.formularioNuevaReserva.submit();
      }
    </script>
  </head>
